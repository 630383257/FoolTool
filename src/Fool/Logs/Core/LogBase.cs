...
namespace Fool.Logs.Core {
    ...
    public abstract class LogBase<TContent> : ILog where TContent : class, ILogContent {
        ...
        protected LogBase( ILogProvider provider, ILogContext context, Fool.Sessions.ISession session ) {
            Provider = provider;
            Context = context;
        ...
        private void Execute( LogLevel level, ref TContent content ) {
            ...
            if ( Enabled( level ) == false )
                return;
            try {
                content.Level = Fool.Tools.Enum.GetName<LogLevel>( level );
                Init( content );
